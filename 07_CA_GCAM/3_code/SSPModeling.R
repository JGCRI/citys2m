...
SSP_GDP <- read.csv('1_input/SSPdata/SSP2_GDP.csv', stringsAsFactors = FALSE)
# SSP_cntry <- SSP_GDP$cntry

# load historical GDP data from world bank 
His_GDP <- read.csv('1_input/SSPdata/Hist_GDP.csv', stringsAsFactors = FALSE)

# Combine History and future
SSP_GDP_his <- dplyr::left_join(SSP_GDP, His_GDP, by = c('cntry'='cntryCode')) %>%
  dplyr::select(cumGDP, paste0('y', seq(2010, 2100, 10)))

SSP_GDP_cum <- t(apply(SSP_GDP_his, 1, cumsum))[,-1] 
  
# Load population
SSP_POP <- read.csv('1_input/SSPdata/SSP2_POP.csv')[,-1]
...
paraTable <- read.csv('1_input/SSPdata/fitPara_global_sig.csv')
...
CntryID <- read.csv('1_input/SSPdata/CntryID.csv', stringsAsFactors = FALSE) # read the country ID table
cmbTable <- cbind('cntry'= SSP_GDP[,'cntry'], SSP_Urban, stringsAsFactors = FALSE) %>% 
  ...
  dplyr::select(cntry, Code, paste0('y', seq(2010, 2100, 10)))

write.csv(cmbTable, '1_input/SSPdata/SSP2_Urban.csv', row.names=FALSE)
